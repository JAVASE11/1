...
PATH_SYMBOLS = "symbols"     #Получить поддерживаемые типы валютных операций
PATH_TICKER = "ticker.do"    #Получить котировки валютной торговли
PATH_TRADES = "trades.do"    #Получить информацию о валютной операции
PATH_DEPTH = "depth.do"      #Получить глубину валютного рынка
PATH_ORDER_INFO = "order_info.do"   #Получить информацию о заказе
PATH_ORDERS_INFO = "orders_info.do"   #Получить информацию о заказе оптом
PATH_ORDER_HISTORY = "order_history.do"   #Получить историческую информацию о заказе, вернуть только информацию за последние два дня
PATH_CANCEL_ORDER = "cancel_order.do"   #Отменить заказ
PATH_BALANCES_USERINFO = "userinfo.do"  #Личные активы
PATH_TRADE = "trade.do"    #Получить информацию о валютной операции
...
class OkexTradeClient(OkexBaseClient):
    ...
    def place_order(self, amount, price, ord_type, symbol='btcusd'):
        ...
        if ord_type not in ('buy', 'sell', 'buy_market', 'sell_market'):
            #Тип торговли: лимит цен (покупка / продажа), рыночный прайс-лист (buy_market / sell_market)
    ...
    def status_order(self, symbol, order_id):
        """
        # Request
        order_id -1: Заказ не был выполнен, в противном случае запрашивается заказ, соответствующий номеру заказа.
        ...
        create_date: Время комиссии
        avg_price: Средняя цена сделки
        deal_amount: Количество транзакций
        order_id:ID заказа
        orders_id:Идентификатор заказа (не рекомендуется)
        price: цена
        status:-1: revoked  0:Не заполнено 1: Часть сделки 2: Завершение сделки 4: Процесс вывода средств
        type:buy_market:Рыночная цена покупки / sell_market: Рыночная цена
        """
        payload = {
    ...
    def cancel_order(self, symbol, order_id):
        ...
        order_id: Идентификатор заказа (несколько идентификаторов заказов разделены знаком «,», что позволяет отозвать до 3 заказов одновременно)
        # Response
        #Несколько результатов возврата заказа (успешный идентификатор заказа, ошибочный идентификатор заказа)
    ...
    def history(self, symbol, status, limit=500):
        ...
        status: Статус запроса 0: Незавершенный заказ 1: Завершенный заказ (данные за последние два дня)
        # Response
        {
            "current_page": 1,
            "orders": [
        ...
        status:-1: Отменено 0: Не заполнено 1: Часть сделки 2: Завершить сделку 4: Процесс вывода средств
        type:buy_market:市价买入 / sell_market:市价卖出
        """
        PAGE_LENGTH = 200 # Okex ограничивает количество данных на странице до 200
        final_result = []
